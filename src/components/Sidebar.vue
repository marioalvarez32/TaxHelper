    ...
    <v-navigation-drawer permanent rounded="lg" rail elevation="5">
      <v-list :lines="false" density="compact" nav class="sidebar__nav-list">
        <template v-for="(menuItem, i) in navigationItems" :key="i">
          <v-list-item v-if="!menuItem.SubItems" :to="menuItem.Path" :active="$route.path === menuItem.Path" active-color="primary">
          ...
          <v-list-group v-else value="Tools">
            <template v-slot:activator="{ props }">
              <v-list-item v-bind="props" :prepend-icon="menuItem.Icon" :active="$route.path === subItem.Path" :title="menuItem.Text"></v-list-item>
            </template>
            <v-list-item v-for="(subItem, i) in menuItem.SubItems" :to="subItem.Path" :key="i" :value="subItem.Text" :title="subItem.Text" :prepend-icon="subItem.Icon"> </v-list-item>
          </v-list-group>
        </template>
